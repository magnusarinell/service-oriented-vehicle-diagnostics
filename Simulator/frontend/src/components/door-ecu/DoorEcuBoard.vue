...
const FAULT_DEFS: Array<{ code: FaultCode; label: string; desc: string }> = [
  { code: 'B1001', label: 'B1001', desc: 'Door Latch' },
  { code: 'B1002', label: 'B1002', desc: 'Window Motor' },
  { code: 'B1003', label: 'B1003', desc: 'CAN Cable Fault' },
]
</script>
...
<template>
  <div class="rounded-xl border border-border bg-card shadow-lg overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-border flex items-center justify-between gap-4">
      <div>
        <h2 class="font-bold text-xl">DoorECU — Door Control Module</h2>
    ...
    <div class="p-6 space-y-6">
      ...
      <div class="flex items-center gap-4">
        ...
        >
          <span>⚠</span> Cable severed — B1003 active
        </div>
      </div>
      ...
      <!-- Active fault list -->
      <div v-if="faultBits.B1001 || faultBits.B1002 || faultBits.B1003">
        <p class="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">
          Active in DoorECU
        </p>
        <ul class="space-y-1">
      ...
      <div v-else class="text-sm text-muted-foreground italic">No active faults in DoorECU</div>
